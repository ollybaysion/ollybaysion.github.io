 ...
 * 글 화면의 길잡이 — 목차와 관련 글.
 *
 * 본문 기둥 양옆에 여백이 넉넉하면 목차는 오른쪽, 관련 글은 왼쪽에 서서 스크롤을 따라
 * 내려오고(`rail`), 모자라면 오른쪽 아래 알약 하나로 접혔다가 누르면 둘이 한 판에 올라온다
 * (`sheet`). 같은 요소가 자리만 바꾼다.
 *
 * 목차는 본문 제목(h2·h3)에서 만든다. 글이 HTML로 쓰였으면 제목에 id가 없어서 여기서 붙인다.
 * 기둥은 zoom을 먹은 채라 자리는 전부 getBoundingClientRect(화면 px)로 잰다.
 */

/** 기둥 양옆 여백이 이만큼은 돼야 옆에 선다. 모자라면 알약으로 접힌다. */
const RAIL_MIN = 180;
const RAIL_MAX = 248;
...
function mount(nav: HTMLElement, body: HTMLElement, head: SVGSVGElement): void {
  ...
  /**
   * 여백을 재서 모습을 정하고, 옆에 선다면 가로 자리를 잡는다. 목차는 기둥 오른쪽 끝에서,
   * 관련 글은 기둥 왼쪽 끝에서 같은 거리만큼 띄운다 — 폭도 둘이 같다(좁은 쪽에 맞춘다).
   */
  function place(): void {
    const view = document.documentElement.clientWidth;
    const column = body.getBoundingClientRect();
    const tocLeft = column.right + RAIL_GAP;
    const relRight = column.left - RAIL_GAP;
    const room = Math.min(view - tocLeft, relRight) - RAIL_MARGIN;
    const next: Mode = room >= RAIL_MIN ? "rail" : "sheet";
    if (next !== mode) setOpen(false);
    mode = next;
    nav.dataset.mode = mode;
    if (mode === "rail") {
      const width = Math.min(RAIL_MAX, room);
      nav.style.setProperty("--pnav-width", `${width}px`);
      nav.style.setProperty("--toc-left", `${tocLeft}px`);
      nav.style.setProperty("--rel-left", `${relRight - width}px`);
    }
    follow();
  }
  ...
  /** 목차가 화면보다 길면 지금 제목이 보이게 목차만 굴린다(창은 건드리지 않는다). */
  function keepInView(li: HTMLElement): void {
    if (toc.scrollHeight <= toc.clientHeight) return;
    const at = li.offsetTop;
    if (at < toc.scrollTop) toc.scrollTop = at - 8;
    else if (at + li.offsetHeight > toc.scrollTop + toc.clientHeight) {
      toc.scrollTop = at + li.offsetHeight - toc.clientHeight + 8;
    }
  }
